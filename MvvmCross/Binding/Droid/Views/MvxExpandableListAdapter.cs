...
namespace MvvmCross.Binding.Droid.Views
{
    public class MvxExpandableListAdapter : MvxAdapter, IExpandableListAdapter
    {
        ...
        {
            return (long) (0x8000000000000000UL | (ulong) ((groupId & 0x7FFFFFFF) << 32) |
                           (ulong) (childId & 0xFFFFFFFF));
        }

        public virtual View GetChildView(int groupPosition, int childPosition, bool isLastChild, View convertView,
            ViewGroup parent)
        {
        ...
        public virtual Tuple<int, int> GetPositions(object childItem)
        {
            ...
            for (var groupPosition = 0; groupPosition < groupCount; groupPosition++)
            {
                var childPosition = ((IEnumerable) GetRawGroup(groupPosition)).GetPosition(childItem);
                if (childPosition != -1)
