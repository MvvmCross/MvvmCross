...
namespace MvvmCross.Droid.Support.V4.EventSource
{
        ...
        : MvxBaseFragmentAdapter
    {
        ...
        protected override void HandleCreateCalled(object sender, MvxValueEventArgs<Bundle> bundleArgs)
        {
            ...
            if (hostMvxView == null)
            {
                MvxAndroidLog.Instance.Warn($"Fragment host for fragment type {Fragment.GetType()} is not of type IMvxAndroidView");
                return;
            }
            ...
            {
                MvxAndroidLog.Instance.Warn($"ViewModel type for Activity {Fragment.Activity.GetType()} not found when trying to show fragment: {Fragment.GetType()}");
                return;
            }
            ...
            {
                MvxAndroidLog.Instance.Warn("Saved state converter not available - saving state will be hard");
            }
            else
            {
                if (bundle != null)
