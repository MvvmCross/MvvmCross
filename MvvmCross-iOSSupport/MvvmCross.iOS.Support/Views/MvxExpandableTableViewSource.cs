...
namespace MvvmCross.iOS.Support.Views
{
    ...
    public abstract class MvxExpandableTableViewSource<TItemSource, TItem> : MvxTableViewSource
        where TItemSource : IEnumerable<TItem>
    {
        ...
        public MvxExpandableTableViewSource(UITableView tableView) : base(tableView)
        {
            _headerButtonCommand = (sender, e) =>
            {
                ...
                foreach (var changedSection in changedSectionsResponse.AllModifiedIndexes)
                {
                    ...
                    var rowCountForSection = RowsInSection(tableView, changedSection);

                    for (var i = 0; i < rowCountForSection; i++)
        ...
        {
            get => isAccordionExpandCollapseEnabled;
            set
            {
        ...
        protected abstract override UITableViewCell GetOrCreateCellFor(UITableView tableView, NSIndexPath indexPath,
            object item);
    }
