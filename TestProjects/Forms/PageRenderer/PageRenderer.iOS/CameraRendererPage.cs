...
namespace PageRendererExample.UI.iOS
{
    public class CameraRendererPage : MvxPageRenderer<CameraRendererViewModel>
    {
        ...
        private void SetupUserInterface()
        {
            ...
            var centerPhotoButton = NSLayoutConstraint.Create(View, NSLayoutAttribute.CenterX, NSLayoutRelation.Equal,
                _takePhotoButton, NSLayoutAttribute.CenterX, 1.0f, 0.0f);
            View.AddConstraint(centerPhotoButton);

            View.AddConstraints(
                NSLayoutConstraint.FromVisualFormat("H:[photoButton(photoButtonWidth)]", 0, viewMetrics));
            View.AddConstraints(
                NSLayoutConstraint.FromVisualFormat("V:[photoButton(photoButtonHeight)]-(photoButtonBottomMargin)-|", 0,
                    viewMetrics));

            View.AddConstraints(
                NSLayoutConstraint.FromVisualFormat("H:|-(cancelButtonMargin)-[cancelButton(cancelButtonWidth)]", 0,
                    viewMetrics));
            View.AddConstraints(
                NSLayoutConstraint.FromVisualFormat("V:[cancelButton(cancelButtonHeight)]-(cancelButtonMargin)-|", 0,
                    viewMetrics));

            View.AddConstraints(NSLayoutConstraint.FromVisualFormat(
                "H:[toggleCameraButton(toggleCameraButtonWidth)]-(toggleCameraButtonRightMargin)-|", 0, viewMetrics));
            View.AddConstraints(NSLayoutConstraint.FromVisualFormat(
                "V:|-(toggleCameraButtonTopMargin)-[toggleCameraButton(toggleCameraButtonHeight)]", 0, viewMetrics));

            View.AddConstraints(NSLayoutConstraint.FromVisualFormat(
                "H:|-(toggleFlashButtonLeftMargin)-[toggleFlashButton(toggleFlashButtonWidth)]", 0, viewMetrics));
            View.AddConstraints(NSLayoutConstraint.FromVisualFormat(
                "V:|-(toggleFlashButtonLeftMargin)-[toggleFlashButton(toggleFlashButtonHeight)]", 0, viewMetrics));
        ...
        private void ToggleFrontBackCamera(object sender, EventArgs e)
        {
            ...
            if (devicePosition == AVCaptureDevicePosition.Front) devicePosition = AVCaptureDevicePosition.Back;
            else devicePosition = AVCaptureDevicePosition.Front;
        ...
        private AVCaptureDevice GetCameraForOrientation(AVCaptureDevicePosition orientation)
        {
            ...
            foreach (var device in devices) if (device.Position == orientation) return device;
        ...
        public override void ViewWillTransitionToSize(CGSize toSize, IUIViewControllerTransitionCoordinator coordinator)
        {
            ...
            if (_videoPreviewLayer.Connection == null) return;
