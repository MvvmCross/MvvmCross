        ...
        : MvxTextBasedRestRequest
    {
        ...
        protected virtual void UploadFields(Stream stream)
        {
            ...
            foreach (var kvp in FieldsToSend)
            {
                ...
                var content = string.Format(formDataTemplate, kvp.Key, kvp.Value);
        ...
        protected virtual void UploadStreams(Stream stream)
        {
            ...
            {
                stream.Write(boundarybytes, 0, boundarybytes.Length);
                var header = string.Format(fileHeaderTemplate, toSend.FieldName, toSend.FileName, toSend.ContentType);
