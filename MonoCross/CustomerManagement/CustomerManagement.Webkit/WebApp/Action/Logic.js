...
// Form Validation
function validateAndPostBlog(formAction) {
    var subject = document.forms['formBlog']['subject'];
    var desc = document.forms['formBlog']['description'];
    
    document.forms['formBlog'].action = formAction;
    
    if (isEmpty(subject, "Please provide a Subject."))
        return false;
    if (isEmpty(desc, "Please provide a Description."))
        return false;
    WA.Submit('formBlog');
    return true;
}

function validateAndPostCalendar(formAction) {
    var event = document.forms['formCalendar']['event'];
    var date = document.forms['formCalendar']['date'];
    var time = document.forms['formCalendar']['time'];
    var desc = document.forms['formCalendar']['description'];
    
    document.forms['formCalendar'].action = formAction;
    
    if (isEmpty(event, "Please provide an Event."))
        return false;
    if (isEmpty(date, "Please provide a Date."))
        return false;
    if (isEmpty(time, "Please provide a Time."))
        return false;
    if (isEmpty(desc, "Please provide a Description."))
        return false;
    WA.Submit('formCalendar');
    return true;
}
