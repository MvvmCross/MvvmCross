...
namespace MvvmCross.CodeAnalysis.Analyzers
{
    ...
    public class AddArgumentToShowViewModelWhenUsingGenericViewModelAnalyzer : DiagnosticAnalyzer
    {
        ...
        internal static readonly LocalizableString Title =
            "Consider passing a parameter to ShowViewModel<T> for the generic form of MvxViewModel<T>";

        internal static readonly LocalizableString MessageFormat =
                "'{0}' inherits from generic 'MvxViewModel<TInit>' or implements 'IMvxViewModelInitializer<TInit>' and requires an argument of type {1}"
            ;

        internal static readonly LocalizableString Description =
                "When calling 'ShowViewModel<TViewModel, TInit>()', if TViewModel inherits from the generic 'MvxViewModel<TInit>' or implements 'IMvxViewModelInitializer<TInit>', then the method call to ShowViewModel should be passing a TInit argument"
            ;

        internal static readonly DiagnosticDescriptor Rule = new DiagnosticDescriptor(
            DiagnosticIds.AddArgumentToShowViewModelWhenUsingGenericViewModelId, Title, MessageFormat, Category,
            DiagnosticSeverity.Warning, true, "Required Matching Type");
        ...
        private static void AnalyzeSyntaxNode(SyntaxNodeAnalysisContext context)
        {
            ...
            {
                var baseInterface = namedSymbol.TypeArguments[0]
                    .AllInterfaces.FirstOrDefault(x => x.MetadataName == MvxViewModelInitializerName);
                    ...
                    baseInterface.TypeArguments.Length == 1)
                {
                    ...
                    if (arguments != null &&
                        ...
                        context.SemanticModel.GetTypeInfo(arguments.Value[0].Expression)
                            .Type.Equals(baseInterface.TypeArguments[0]))
                        return;

                    context.ReportDiagnostic(Diagnostic.Create(Rule, genericNameSyntax.GetLocation(),
                        namedSymbol.TypeArguments[0].Name, baseInterface.TypeArguments[0].Name));
                }
            }
        }
    }
