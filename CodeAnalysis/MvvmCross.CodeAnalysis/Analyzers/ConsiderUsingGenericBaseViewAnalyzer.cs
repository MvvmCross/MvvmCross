...
namespace MvvmCross.CodeAnalysis.Analyzers
{
    ...
    public class ConsiderUsingGenericBaseViewAnalyzer : DiagnosticAnalyzer
    {
        ...
        internal static readonly LocalizableString Title =
            "Consider deriving from generic base class with strongly typed ViewModel property";

        internal static readonly LocalizableString MessageFormat =
            "Derive from '{0}<T>', which already implements a strongly typed ViewModel property";

        internal static DiagnosticDescriptor Rule =
            new DiagnosticDescriptor(DiagnosticIds.UseGenericBaseClassRuleId, Title, MessageFormat, Category,
        ...
        private static void AnalyzeSymbol(SymbolAnalysisContext context)
        {
                ...
                namedSymbol.GetMembers(ViewModelPropertyName).Any())
            {
                var viewModelProperty =
                    namedSymbol.GetMembers(ViewModelPropertyName).FirstOrDefault() as IPropertySymbol;
        ...
        private static bool IsViewModelType(SymbolAnalysisContext context, ITypeSymbol symbol)
        {
            ...
            var iNotifyPropertyChangedType =
                context.Compilation.GetTypeByMetadataName(typeof(INotifyPropertyChanged).FullName);
