...
namespace MvvmCross.CodeAnalysis.CodeFixes
{
    [ExportCodeFixProvider(LanguageNames.CSharp, Name =
        nameof(MvxMessengerSubscriptionDoesntStoreTokenInVariableCodeFix))]
    [Shared]
    public class MvxMessengerSubscriptionDoesntStoreTokenInVariableCodeFix : CodeFixProvider
    {
            ...
            CancellationToken cancellationToken)
        {
            ...
            var tokenField = SyntaxFactory.FieldDeclaration(
                    SyntaxFactory.VariableDeclaration(SyntaxFactory.IdentifierName("MvxSubscriptionToken"))
                        .WithVariables(
                            SyntaxFactory.SingletonSeparatedList(
                                SyntaxFactory.VariableDeclarator(SyntaxFactory.Identifier("_token")))))
            ...
            var storeResultInTokenCall = SyntaxFactory.AssignmentExpression(SyntaxKind.SimpleAssignmentExpression,
                identifierSyntax, ignoredSubscribeCall);
            ...
            if (classDeclarationSyntax != null)
            {
                ...
                {
                    var name = SyntaxFactory.QualifiedName(SyntaxFactory.QualifiedName(
                        SyntaxFactory.IdentifierName("MvvmCross"),
