...
namespace MvvmCross.Core.Navigation
{
    public class MvxNavigationService : IMvxNavigationService
    {
        ...
        protected bool TryGetRoute(string url, out KeyValuePair<Regex, Type> entry)
        {
            try
            {
                ...
                switch (matches.Count)
                {
                    case 0:
                        ...
                        MvxLog.Instance.Trace("MvxNavigationService", "Unable to find routing for {0}", url);
                        return false;
                    case 1:
                        entry = matches[0];
                        return true;
                ...
                MvxLog.Instance.Warn("MvxNavigationService",
            ...
            {
                MvxLog.Instance.Error("MvxNavigationService", "Unable to determine routability: {0}", ex);
                entry = default(KeyValuePair<Regex, Type>);
                return false;
            }
        }
