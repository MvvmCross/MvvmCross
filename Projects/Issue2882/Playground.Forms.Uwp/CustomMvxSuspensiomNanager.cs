...
{
    // Regarding issue #2882
    // this class it's perfectly valid (exact copy of the original MvxSuspensionManager)
    // but will application will crash when suspending/resuming 
    // because MvvmCross.Platforms.Uap.Views.MvxApplication has the following code:
    //
    //    private async void OnSuspending(object sender, SuspendingEventArgs e)
    //    {
    //    var deferral = e.SuspendingOperation.GetDeferral();
    //
    //    var suspension = Mvx.GetSingleton<IMvxSuspensionManager>() as MvxSuspensionManager; // <=== casting to MvxSuspensionManager
    //    await Suspend(suspension);
    //    await suspension.SaveAsync(); // <== NullReferenceException
    //    deferral.Complete();
    //    }
    //
    ...
    //    await suspension.RestoreAsync(); // <== NullReferenceException
    //    }

    public class CustomMvxSuspensionManager : IMvxSuspensionManager
    {
        ...
        public async Task SaveAsync()
        {
            try
            {
                ...
                var file = await ApplicationData.Current.LocalFolder.CreateFileAsync(SessionStateFilename,
        ...
        private readonly DependencyProperty MvxFrameSessionStateKeyProperty =
            DependencyProperty.RegisterAttached("_MvxFrameSessionStateKey", typeof(string),
                typeof(MvxSuspensionManager), null);
        ...
        private readonly List<WeakReference<IMvxWindowsFrame>> _registeredFrames =
            new List<WeakReference<IMvxWindowsFrame>>();
